...
def apply_index_with_cache(image, cache, levels=16):
...
def quantize_color(bgr, levels=16):
...
def build_color_cache(lut_lab, levels=16):
    cache = {}
    for r in range(0, 256, 256 // levels):
        for g in range(0, 256, 256 // levels):
            for b in range(0, 256, 256 // levels):
                quantized_bgr = quantize_color((b, g, r), levels)
                lab = bgr_to_lab_colour_science(quantized_bgr)
                closest_index = get_lut_index_with_min_delta_e(*quantized_bgr, lut_lab)
                cache[quantized_bgr] = closest_index
    return cache
...
if __name__ == "__main__":
    ...
    logging.info("Reading downsampled LUT and building color cache...")
    downsampled_lut_lab = read_lut_and_convert_to_lab(downsample_lut_filename)
    color_cache = build_color_cache(downsampled_lut_lab)
    logging.info("Done building color cache.")
    
    logging.info("Applying LUT to image...")
    image = cv2.imread(image_filename, cv2.IMREAD_COLOR)
    apply_index_with_cache(image, downsampled_lut_lab, color_cache)
